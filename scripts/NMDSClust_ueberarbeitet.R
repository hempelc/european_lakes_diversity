...
spefileprn <- "kingdom_Euk_without_Metazoa_and_Embryophyta_of_dataset_christopher_lakes_data_OTUs_zusammengefasst_lakesumlevel_bigger_than_100_otusumlevel_bigger_than_2.csv"
...
if (pa_flag==F & rrare) sperr <- rrarefy100(round(spe, digits=0),rlev) else sperr <- spe
...
#plot(spe.nmds, main="Goodness of fit", type="n")
plot(spe.nmds, main="Goodness of fit", display="sites", cex=gof*200)
...
titletext <- paste("NMDS(stress:",stress,", dim:",dim_num,", rlev:",rlev,
                   ") + Ward-clusters"," (",dist_text," dist.)",sep="") #spefileprn,
