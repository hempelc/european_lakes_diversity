...
#	par(mfrow=c(1,2))
  ...
  arrcol <- "red"
	...
	plot(res.pca, choices=c(ax1, ax2), display=c("wa", "sp"), type="n", 
		#main="PCA - scaling 1: species scores scaled to relative eigenvalues", scaling=1)
		main="PCA", xlim=c(min(spe.sc1[,ax1]), max(spe.sc1[,ax1])), ylim=c(min(spe.sc1[,ax2]), max(spe.sc1[,ax2])))
	
	if (point) 
	{ for (i in 1:length(grl))
	       points(sit.sc1[gr==i,ax1], sit.sc1[gr==i,ax2],cex=0.8, pch=pchvec[i], bg=colorvec[i])
	  text(res.pca, display="wa", choices=c(ax1 ,ax2), cex=0.6, pos=3,scaling=1)
	}	
	else
	{
	...
	text(res.pca, display="sp", choices=c(ax1, ax2), cex=cex, pos=4, 
		col=arrcol, scaling=1)
	arrows(0, 0, spe.sc1[,ax1], spe.sc1[,ax2], length=0.1, angle=20, col=arrcol)
	pcacircle(res.pca)
...
#	sit.sc2 <- scores(res.pca, display="wa", choices=c(1:p))
	...
	#plot(res.pca, choices=c(ax1,ax2), display=c("wa","sp"), type="n", 
	#	main="PCA - scaling 2: site scores scaled to relative eigenvalues")

	#if (point) 
	#{ for (i in 1:length(grl))
	#       points(sit.sc2[gr==i,ax1], sit.sc2[gr==i,ax2],cex=0.8, pch=pchvec[i], bg=colorvec[i])
	#  text(res.pca, display="wa", choices=c(ax1 ,ax2), cex=0.6, pos=3)
  # }	
	#else
	#{
	#	text(res.pca, display="wa", choices=c(ax1, ax2), cex=cex)
	#}
	#text(res.pca, display="sp", choices=c(ax1, ax2), cex=cex, pos=4, col=arrcol,scaling=2)
	#arrows(0, 0, spe.sc2[,ax1], spe.sc2[,ax2], length=ahead, angle=20, col=arrcol)
}
...
# the 'const' value below.
	...
	radius <- (2/p)^0.5
	radius <- radius * const
	symbols(0, 0, circles=radius, inches=FALSE, add=TRUE, fg="darkcyan")
}
