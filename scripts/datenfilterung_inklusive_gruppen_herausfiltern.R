...
data.entry(filt_kingd,del_group1,del_group2,excl1_flag,exclusive_group1,
...
if (write_exclusive_file) 
  ...
  write.csv2(exclusive_groups, file=paste(write_name, ".csv", sep=""), row.names=F)
  write.csv2(exclusive_groups_tax, file=paste(write_name, ".tax", sep=""), row.names=F)
  
  }
# end write excl file


# chunk13: write double filtered abundance table and tax table
outfilename <- paste("kingdom", filt_kingd, "without", del_group1, "and", del_group2, "of_dataset", filename, "lakesumlevel_bigger_than", lakesumlevel, "otusumlevel_bigger_than", otusumlevel, sep="_")
out1 <- paste(outfilename,".csv",sep="")
out2 <- paste(outfilename,".tax",sep="")
write.csv2(spe,file=out1,row.names=F)
write.table(tax,file=out2,row.names=F, col.names=T, dec=".",sep="\t", quote=FALSE)

# end chunk13


# chunk 14: write IND file if write_IND=T

if (write_IND)
  {  
  rn <- paste("N",1:nrow(spe),sep="") # =row.names(spe)
  ...
  rs <- rowSums(spe)
  taxshort <- strsplit(as.character(tax),";")
  taxshort <- sapply(taxshort,lastitems)
  taxshort <- paste(rn,"_",rs,"_",taxshort,sep="")
  spe_ind <- cbind(taxshort,spe)
  indfilename <- paste("kingdom", filt_kingd, "without", del_group1, "and", del_group2, "of_dataset", filename, "lakesumlevel_bigger_than", lakesumlevel, "otusumlevel_bigger_than", otusumlevel, sep="_") # for indicspec.R
  indfilename_plus <- paste(indfilename,".ind",sep="")
  write.table(spe_ind,file=indfilename_plus,row.names=F, col.names=T, dec=".",sep="\t", quote=FALSE)
  }
