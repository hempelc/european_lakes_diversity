...
rlevel <- F
cat("chosen rlevel = ",rlevel,"\n","Extrapol.method based on: ACE","\n\n",sep="")
...
data.entry(filename,revord,Inclunknown,Inclothers,Sortsubs,sortglob)
...
aggfilename <- paste(filename,".agg",sep="")
...
spedrare <- drarefy(spe,rlevel)
spedrare <- extraspedrare(spe,spedrare,rlevel,chao=F)

spe_rich      <- rowSums(spedrare)  # ALMOST identical to rarefy(spe,rlevel), but extraspe


colorvec <- c("red","green","darkblue","cyan","darkred",
...
taxkey2 <- c(G1,G2,G3,G4,
             G5,G6,G7,G8,G7,G7,G10,G11,
             G12,G13,
             G14,G15,G16,G17,G18,G18,G18,G18,G18,G18,G18,G18,
             G19,G20,G21,G22,G23,G24,G0,
             G25,
             G26,
             G27,G28,G0,G29,
             G30,G31,
             G32,G0,G0,G0,G0,G0,G0,G0,G0,
             GREST,GREST,GREST,GREST)
...
spedrare.trans <- t(spedrare)
spedrare.trans <- subset(spedrare.trans,taxlist[[1]] != "N")
spe.trans <- t(spe)
...
spedrare <- t(spedrare.trans)
spe <- t(spe.trans)
spe_rich <- rowSums(spedrare)
...
spedrare      <- spedrare[order(index),]
spe           <- spe[order(index),]
spe_otu_rich.trans <- spe_otu_rich.trans[,order(index)]
spedrare.trans <- spedrare.trans[,order(index)]
...
if (Inclunknown==F & Inclothers==F)
            write.table(spe.trans,paste(filename,".pNO",sep=""),row.names=T, col.names=T, 
            dec=".",sep="\t", quote=FALSE)

t.spedrare.agg <- aggregate(spedrare.trans,taxlist,sum)
...
spedrare.agg <- t(t.spedrare.agg)
write.table(spedrare.agg,file=aggfilename,row.names=T, col.names=F, 
            dec=".",sep="\t", quote=FALSE)
spedrare.agg <- read.csv(aggfilename,row.names=1, nrows=500, sep="\t") 
...
#  writeagg <- writeprop(spedrare.agg,"_RARE")
#}  
...
spe.table <- as.table(as.matrix(spedrare.agg))
xmax <- max(rowSums(spedrare.agg))+5
xlimits <- c(0,xmax)
...
textlegends <- as.vector(dimnames(spe.table)[[2]])
maintitle <- paste("rarefaction at: ",as.character(rlevel))
pienums <- colSums(spedrare.agg)
textrotation <- 0
