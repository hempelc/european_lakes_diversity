...
filename <- "file:///M:/Allgemeine Daten/Christopher/Bachelor/Christopher-20Seen/&Eukbest151102-103935-final_swoncbi85_tax_(just_Eukaryotes_without_Metazoa_Embryophyta)"
#filename <- "Alpine_world" # for filtered abundance table....
# end filenames
...
spefiletax <- paste(filename,"_TaxPrnInd_ncbi.tax",sep="") #for TAX information ...
spefileprn <- paste(filename,"_TaxPrnInd_ncbi.prn",sep="") #for clean abundance file
indfilename <- paste(filename,"_TaxPrnInd_ncbi.ind",sep="") # for indicspec.R
...
# chunk5: clear unnecessary columns within spe and store sample/otu numbers
if (names(spe)[2]=="taxlevel") spe <- select(spe, -c(seqid:seqs)) else select(spe,-seqs,-taxonomy)
orig_cols <- ncol(spe) # original number of samples
orig_otus <- nrow(spe) # original number of otus
# end clear spe-columns
