...
# modified (p_valuel line 19) by Manfred Jensen, Biodiv group, Uni-Duisburg-Essen, 24 May 2016
...
panel.cor <- function(x, y, method="pearson", digits=3, cex.cor=1.2, no.col=FALSE)
{
	...
	if(ra <= 0.01) prefix <- "**"
	if(ra <= 0.001) prefix <- "***"
	p_ra <- paste("p=",round(ra,digits=5),sep="")
	if(no.col)
	{
		color <- 1
